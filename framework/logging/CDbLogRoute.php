...
class CDbLogRoute extends CLogRoute
{
	...
	protected function processLogs($logs)
	{
		...
		{
            $data = array(
                'LogLevel_Severity'=>$log[1],
                'Error_Category'=>$log[2],
                'Error_DateTime'=>@date('Y-m-d H:i:s',$log[3]),
                'Error_Msg'=>$log[0],
                'Application'=>$_SERVER['HTTP_USER_AGENT'],
            );
            ...
            if (isset(Yii::app()->user->userID)) {
                $data['User_Login'] = Yii::app()->user->userID;
            }

			$command->insert($this->logTableName,$data);
		}
	}
}
