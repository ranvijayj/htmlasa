...
class Images extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('Document_ID, File_Name, Mime_Type, Img, Pages_Count , File_Size', 'required'),
			array('Document_ID, File_Size', 'numerical', 'integerOnly'=>true),
			array('File_Name, Mime_Type', 'length', 'max'=>255),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('Image_ID, Document_ID, File_Name, Mime_Type, Img,Pages_Count, File_Size', 'safe', 'on'=>'search'),
		);
	}
    ...
    /**
     * Gets 20 last images of  the user to current client
     * @return mixed
     */
    public function getLastUsersImages()
    {
        $sql = "SELECT `File_Name`, `Mime_Type`, `images`.`Document_ID` as Document_ID FROM
                   `images`
                LEFT JOIN `documents` ON `documents`.`Document_ID` = `images`.`Document_ID`
                WHERE `documents`.`User_ID` = '" . Yii::app()->user->userID . "' AND
                      `documents`.`Client_ID` = '" . Yii::app()->user->clientID . "'";

        if (Yii::app()->user->projectID !== 'all') {
            $sql .= " AND `documents`.`Project_ID` = '" . Yii::app()->user->projectID . "' ";
        }

        $sql .=  " ORDER BY `images`.`Image_ID` DESC
            LIMIT 20
        ";
    ...
    /**
     * Gets string that will be appended to LastUpload section in upload mudule
     * @return mixed
     */
    public static function getAjaxStringForLastUploadSection($doc_id)
    {
        $sql = "SELECT `File_Name`, `Mime_Type`, `Document_ID` FROM
                   `images`
        ...
        $type = explode('/', $rows[0]['Mime_Type']);
        $type = $type[1];

        $result= "<tr><td data-id='" . $rows[0]['Document_ID'] . "'><img src='" . Yii::app()->request->baseUrl . "/images/file_types/" . $type . ".png' alt='" . strtoupper($type) . "' class='img_type' />" . CHtml::encode($rows[0]['File_Name']) . "</td></tr>";

        return $result;
    }

    /**
     * Get Count of Uses storage by client
     * @param $clientID
     * @return int
     */
    public static function getUsedStorage($clientID)
    {
        $usedStorage = 0;
        $sql = "SELECT sum(File_Size) as used_storage FROM
                   `images`
                LEFT JOIN `documents` ON `documents`.`Document_ID` = `images`.`Document_ID`
        ...
        if ($result) {
            $usedStorage = $result['used_storage']/(1024*1024*1024);
        }
    ...
    /**
     * Find First text string in pdf
     * @param $infile
     * @return string
     */
    public function findPdfText($infile) {
        ...
        // много всего интересного, например, те же шрифты.
     ...
     * @return string
     */
    public function pdf2text($infile) {
        // Читаем данные из pdf-файла в строку, учитываем, что файл может содержать
        // бинарные потоки.
        if (empty($infile))
            return "";
    ...
    public function getObjectOptions($object) {
        ...
        // находся между  << и >>. Каждая опция начинается со слэша /.
        $options = array();
    ...
    /**
     * Returns image from database is file with $filepath has a dublicate in database
     * @param $filepath
     * @return bool
     */
    public static function checkHashDublication($filepath){

        if (file_exists($filepath)){
            $hashUploaded = sha1_file($filepath);
            $condition = new CDbCriteria();
            $condition->select = 't.Document_ID, File_Hash, Mime_Type, File_Name';
            ...
            $condition->addCondition("t.File_Hash='" . $hashUploaded . "'");
            $image = Images::model()->find($condition);
            if ($image) {
                return $image;
            } else {
                return false;
