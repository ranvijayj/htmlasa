...
?>
<h1>AP Approval List: <?=@CHtml::encode(Yii::app()->user->userLogin);?><span class="right items_to_review" id="items_to_review" data-id="<?php echo count($apList); ?>"> AP to Approve: <span id="number_items"><?php echo count($to_be_approved_count); ?></span> items</span></h1>
...
<div class="account_manage">
    <div class="account_header_left left">
        ...
        <button class="button right" id="submit_ap_form">View Doc</button>
    </div>
    <div class="right">
        <span class="left search_block_label">Search: &nbsp;</span>
        <div class="search_block">
            <input type="text" name="search" value="<?php echo $searchQuery; ?>" id="search_field" maxlength="250" maxlength="250" autocomplete="off">
            <div id="search_options">
               <span class="search_options_header">Search in the fields:</span><br/>
                ...
                $options = array(
                    ...
                    'search_option_date' => array('AP Date(Invoice Date)', 0),
                );
        ...
        $this->widget('CTabView', array(
            ...
            'tabs'=>array(
                'tab1'=>array(
                    'title'=>'Payable List',
                    'view'=>'tabs/ap_list',
                    'data'=> array('apList' => $apList, 'notes' => $notes),
                ),
            ),
        ));
        ?>
...
<? } ?>
<?php $this->renderPartial('application.views.widgets.batch_form', array('userSettings' => $userSettings, 'docType' => 'AP')); ?>
<script>
    $(document).ready(function() {
        setEqualHeight($(".wrapper,.sidebar_right"));
        var listClass = new ApList;
        new BatchExporting('<?php echo Documents::AP; ?>', listClass);
    });
</script>
<script src="<?php echo Yii::app()->request->baseUrl; ?>/js/w9_list.js"></script>
<script src="<?php echo Yii::app()->request->baseUrl; ?>/js/ap_list.js"></script>
