...
<div class="account_manage">
    <div class="account_header_left left">
        ...
        <? if ($hard_approval && !$mustBeApproved ) {?>
            <button class="button" id="hard_approve" data="<?php echo $document->Document_ID; ?>">Apprv./Skip</button>
        <?}?>
    ...
    $this->widget('CTabView', array(
        ...
        'tabs'=>array(
            'tab1'=>array(
                'title'=>'Payable View',
                'view'=>'tabs/ap_view',
                'data'=>array('file' => $file, 'document' => $document, 'approved'=>$ap->Approved),
            ),
            'tab2'=>array(
                'title'=>'Backup View',
                'view'=>'tabs/detail_back_up',
                'data'=>array('backUp'=>$backUp,'approved'=>$ap->Approved),
            ),
            'tab3'=>array(
                'title'=>'Check View',
...
<div class="sidebar_right">
    <div class="sidebar_item approval_progress">
        ...
        <br/>
        <div id="progress_bar" data-id="<?=$ap->Document_ID;?>">
...
<?php
if ($enableEditing) {
    $this->renderPartial('application.views.widgets.ap_dists', array(
        'dists' => $dists,
        'invalidDistsSum' => $invalidDistsSum,
         'clickable' => $user_can_edit
        )
    );
...
<script>
    $(document).ready(function() {
        ...
        <?php
        //    echo (!$backUp['document']) ? "new BuUpload('ap');" : '';
        ?>
    });
</script>
<?php
